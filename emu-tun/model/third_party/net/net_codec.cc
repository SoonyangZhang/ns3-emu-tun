...
:DataReader(buf,len,NETWORK_BYTE_ORDER){}
...
bool DataReader::ReadUInt16(uint16_t *result){
    ...
    if(endianness_ == NETWORK_BYTE_ORDER){
        *result=QuicheEndian::NetToHost16(*result);
    }
    return true;
}
bool DataReader::ReadUInt32(uint32_t *result){
    ...
    if(endianness_ == NETWORK_BYTE_ORDER){
        *result=QuicheEndian::NetToHost32(*result);
    }
    return true;
}
...
bool DataReader::ReadBytesToUInt64(uint32_t num_len,uint64_t *result){
    *result=0u;
    if(HOST_BYTE_ORDER==endianness_){
...
:DataWriter(buf,len,NETWORK_BYTE_ORDER){
}
...
bool DataWriter::WriteUFloat16(uint64_t value){
  ...
  if (endianness_ == NETWORK_BYTE_ORDER) {
    result = QuicheEndian::HostToNet16(result);
  }
